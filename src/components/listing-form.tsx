  ...
  if (done) {
    return (
      <div className="rounded-lg border border-border bg-card p-6">
        <p className="font-heading text-2xl font-semibold">Listing request in</p>
        <p className="mt-2 text-sm text-muted-foreground">
          We will only publish public facts you confirm. Truckpay will not
          invent pay figures or reviews for your firm.
        </p>
      </div>
    );
  }
    ...
    <form onSubmit={onSubmit} className="space-y-4 rounded-xl bg-card p-5 ring-1 ring-foreground/10">
      ...
      <label className="block space-y-1.5">
        ...
        <textarea
          ...
          placeholder="HQ, lanes, equipment. Do not send unverified pay claims."
        />
      </label>
      {error ? <p className="text-sm text-destructive">{error}</p> : null}
      <Button type="submit" disabled={pending}>
        {pending ? "Sending…" : "Request a listing"}
      </Button>
    </form>
  );
}
